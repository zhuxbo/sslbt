...
- SetSSL 部署：写入前 pre-flight 校验既有配置（损坏即快速失败不写入）并捕获原证书；结果白名单判定（仅 dict 且 status is True 算成功）；写入后 checkWebConfig + serviceReload 校验失败则回滚原证书；失败回调 failure 经 message 携带原因（含回滚状态，客户端脱敏并截断至 256 字符，success 不带）
