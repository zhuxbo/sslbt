...
- GitHub-only 附加资产：无。
- 服务器级可变引导文件：`deploy/install.sh`，发布时同步到每个节点根目录，但不属于版本资产和 `checksums`。
...
- `bash build/release.sh --stage BUNDLE`：不构建；要求各节点索引语义一致，生成一次候选索引，将同一 ZIP、installer、manifest、候选索引暂存到全部节点并校验 SHA256。main 要求公开版本目录尚不存在。
- `bash build/release.sh --publish BUNDLE`：不构建；main 要求本地 `v{version}` 精确指向 manifest commit。先在全部节点准备资产，再原子替换各节点索引；失败时尝试把所有已处理节点恢复到原索引和目录。
...
发布配置 `SERVERS` 的每个节点都是正式一致性范围。stage 前各节点 `releases.json` 必须语义一致；候选索引只生成一次并随 bundle 保存。publish 先让全部节点拥有相同版本字节，再推进可见索引。任何失败都不得报告成功；保留 bundle、manifest、候选索引、远端 `.staging/`/`.rollback/` 证据，修复节点后对 main 使用 `--resume`。

版本 tag 创建前失败可删除未公开的暂存事务后重新 stage；tag 创建后不得重新 `--prepare`、不得移动 tag、不得用新构建替换 bundle。已完成正式版只能用更高版本修复。
